...
def render_flow_guide():
    ...
    html = (
        ...
        '<li>ارفع الملف بعد التنظيف في خانة "إعادة الرفع" — البرنامج يتحقق من جديد. يجب أن يكون العدد صفر.</li>'
        '<li>حدد الدرجة الكبرى لكل سؤال (البرنامج يعبئ المتاح تلقائياً).</li>'
        '<li>اضغط زر التقسيم — سيتم تحميل الملفات ZIP جاهزة للمصححين.</li>'
        '</ol>'
        '</div>'
    )
    st.markdown(html, unsafe_allow_html=True)
          ...
          if not all_points_items:
              ...
              if st.button("💾 حفظ قالب الدرجات لهذا الملف"):
                  if can_split:
                      ...
                      st.error("يجب أن تكتب الدرجة الكبرى لكل أعمدة Points قبل الحفظ.")
